document.addEventListener('DOMContentLoaded', () => {
  ...
  const order = JSON.parse(localStorage.getItem('PlantShopOrder') || 'null');
  ...
  if (communityDetailsElement && order) {
    communityDetailsElement.innerHTML = `
      <h3 class="text mt-3">Your Order Summary</h3>
      <ul class="list-unstyled">
        ...
        <li><strong>Additions:</strong> ${
          order.Additions && order.Additions.length ? order.Additions.join(', ') : 'None'
        }</li>
  ...
  if (cartDetailsElement) {
    if (order) {
      cartDetailsElement.innerHTML = `
        ...
        <h3 class="text mt-4">Items Ordered:</h3>
        <ul class="list-unstyled">
          <li><strong>Size:</strong> ${order.size || ''}</li>
          <li><strong>Base:</strong> ${order.base || ''}</li>
          <li><strong>Additions:</strong> ${
            order.Additions && order.Additions.length > 0 ? order.Additions.join(', ') : 'None'
          }</li>
          ${order.notes ? `<li><strong>Notes:</strong> ${order.notes}</li>` : ''}
        </ul>
        <h3 class="text mt-4">Order Summary:</h3>
        <ul class="list-unstyled">
          <li><strong>Subtotal:</strong> $${order.subtotal || '0.00'}</li>
          <li><strong>Tax:</strong> $${order.tax || '0.00'}</li>
          <li><strong>Delivery Fee:</strong> $${order.deliveryFee || '0.00'}</li>
    ...
    if (clearCartBtn) {
      clearCartBtn.addEventListener('click', () => {
        localStorage.removeItem('PlantShopOrder');
  ...
  if (orderForm) {
    orderForm.addEventListener('submit', (event) => {
      ...
      localStorage.setItem('', JSON.stringify(nextOrder));
  ...
  if (orderNumberElement && orderDetailsElement) {
    ...
    const payOrder = JSON.parse(localStorage.getItem('PlantShopOrder') || 'null');
    if (payOrder) {
      orderDetailsElement.innerHTML = `
        ...
        <ul class="list-unstyled">
          ...
          <li><strong>Additions:</strong> ${
            payOrder.Additions && payOrder.Additions.length > 0
              ? payOrder.Additions.join(', ')
              : 'None'
          }</li>
          ${payOrder.notes ? `<li><strong>Notes:</strong> ${payOrder.notes}</li>` : ''}
        </ul>
        <h3 class="text mt-4">Payment Summary:</h3>
        <ul class="list-unstyled">
  ...
  if (registrationForm) {
    registrationForm.addEventListener('submit', (event) => {
      ...
      localStorage.setItem('PlantShopUserProfile', JSON.stringify(userProfile));
